...
def apply_clean_architecture_project_hierarchy(sln_path, proj_name):        
    ...
    subprocess.run(["dotnet", "new", "classlib", "-n", f"{proj_name}.Domain", "-f", "net7.0"], stdout=subprocess.DEVNULL, shell=False)
    subprocess.run(["dotnet", "new", "classlib", "-n", f"{proj_name}.Infrastructure", "-f", "net7.0"], stdout=subprocess.DEVNULL, shell=False)
